...
void cv_apply_median_filter(Image *img, int size) {
    ...
    unsigned char *tempBytes =
        (unsigned char *)malloc(img->width * img->height * ch * sizeof(unsigned char));

    for (int i = 0; i < img->height; ++i) {
        for (int j = 0; j < img->width; ++j) {
            unsigned char *pixelValues =
                (unsigned char *)malloc(size * size * ch * sizeof(unsigned char));
            int pixelCount = 0;
            ...
            for (int k = 0; k < size; ++k) {
                for (int l = 0; l < size; ++l) {
                    ...
                    if (row >= 0 && row < img->height && col >= 0 && col < img->width) {
                        for (int c = 0; c < ch; ++c) {
                            pixelValues[pixelCount++] =
                                img->bytes[(row * img->width + col) * ch + c];
                        }
                    }
                }
            }
