...
void cv_apply_gaussian_blur(Image *image, float sigma, int size) {
    ...
    unsigned char *newImageBytes =
        malloc(image->height * image->width * image->channels * sizeof(unsigned char));
            ...
            for (int c = 0; c < image->channels; c++) {
                ...
                for (int k = 0; k < size; k++) {
                    for (int l = 0; l < size; l++) {
                        ...
                        if (yIndex >= 0 && yIndex < image->height && xIndex >= 0 &&
                            xIndex < image->width) {
    ...
    memcpy(image->bytes, newImageBytes,
           image->height * image->width * image->channels * sizeof(unsigned char));
    free(newImageBytes);
}
