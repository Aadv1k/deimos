...
void cv_apply_grayscale(Image *img) {
    ...
    unsigned char *grayBytes =
        (unsigned char *)malloc(img->width * img->height * sizeof(unsigned char));

    for (int i = 0, j = 0; i < img->width * img->height * img->channels; i += img->channels, j++) {
        const unsigned char R = img->bytes[i], G = img->bytes[i + 1], B = img->bytes[i + 2];
